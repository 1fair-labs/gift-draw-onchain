...
Findings from the 2026-06 transparency review and what was done about them. The findings were raised
against spec **v19**; the current spec is **v21**. v20 changed the prize pool split; v21 mixed the hash
of a future slot into the seed and widened shuffle keys to 53 bits (see `SETTLEMENT-SPEC.md`). Neither
changed the hash input, so every response below still describes the code that runs today.
