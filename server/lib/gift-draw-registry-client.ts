...
export const SETTLEMENT_SPEC_VERSION_ON_CHAIN = 18;
...
export async function sendDrawRandomnessTx(params: {
  ...
  const tx = await program.methods
    .drawRandomness(params.drawId, params.periodEndIso, params.merkleRootHex)
    .accounts({
      authority: sponsor.publicKey,
...
export async function parseKindRolledFromTx(
  ...
  if (parsedEvents.length === 0) {
    if (lastDecodeName) {
      console.warn('parseKindRolledFromTx: decoded event not mapped', {
        signature,
