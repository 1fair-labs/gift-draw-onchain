...
/**
 * The GIFT/USDC pool's trade fee (Raydium CPMM, 0.25%). A USDC ticket is swapped through the pool,
 * so what lands in the draw is $1 of GIFT minus this fee.
 */
export const POOL_TRADE_FEE_RATE = 0.0025;

/**
 * Price of one ticket in the chosen currency.
 *
 * GIFT is priced as what $1 buys through the pool — spot minus the pool fee — so a ticket carries the
 * same GIFT value however it was paid for, and the Mint button shows the number the ticket will.
 * Nobody is charged a fee here: a GIFT payer simply hands over exactly what a USDC payer's dollar
 * turns into. No swap happens for GIFT, so the pool price does not move.
 */
export function getTicketUnitPrice(
  ...
  return (TICKET_PRICE_USDC / giftUsdcSpotPrice) * (1 - POOL_TRADE_FEE_RATE);
}
