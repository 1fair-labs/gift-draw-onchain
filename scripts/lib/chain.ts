...
/** `["draw_entropy", drawId]` — spec v21, `DrawEntropy` in programs/gift_draw_registry/src/lib.rs. */
export function drawEntropyPda(drawId: string, program: PublicKey): PublicKey {
...
function anchorDiscriminator(namespace: 'account' | 'event' | 'global', name: string): Buffer {
...
/**
 * `DrawEntropy` (spec v21): draw_id_len u8 | draw_id [u8;32] | target_slot u64 | slot u64 |
 * slot_hash [u8;32] | seed [u8;32] | revealed bool | rearm_count u8 | bump u8
 */
export async function fetchDrawEntropy(drawId: string): Promise<DrawEntropyAccount | null> {
  const pda = drawEntropyPda(drawId, programId());
  const info = await connection().getAccountInfo(pda);
  if (!info) return null;
  const d = Buffer.from(info.data);
  requireDiscriminator(d, 'account', 'DrawEntropy');
  const idLen = d.readUInt8(8);
  return {
    pda: pda.toBase58(),
    drawId: d.subarray(9, 9 + idLen).toString('utf8'),
    targetSlot: d.readBigUInt64LE(41),
    slot: d.readBigUInt64LE(49),
    slotHashHex: d.subarray(57, 89).toString('hex'),
    seedHex: d.subarray(89, 121).toString('hex'),
    revealed: d.readUInt8(121) === 1,
    rearmCount: d.readUInt8(122),
  };
}

export type SealTransaction = {
  signature: string;
  slot: number;
  drawId: string;
  periodEndIso: string;
...
function readBorshString(d: Buffer, at: number): [string, number] {
  const len = d.readUInt32LE(at);
  return [d.subarray(at + 4, at + 4 + len).toString('utf8'), at + 4 + len];
}

const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

/**
 * The `draw_randomness` transaction that created a draw's `DrawSeed`: the slot it landed in and the
 * arguments it was called with. Other transactions reference the same address too (the draw vault
 * names it on every entry, before the seed exists), so the history is walked newest-first — only
 * the reveal and the commit come after the seal.
 */
export async function fetchSealTransaction(drawId: string): Promise<SealTransaction | null> {
  const pda = drawSeedPda(drawId, programId());
  const c = connection();
  const tag = anchorDiscriminator('global', 'draw_randomness');
  let before: string | undefined;
  for (;;) {
    const page = await c.getSignaturesForAddress(pda, { before, limit: 100 });
    for (const s of page) {
      if (s.err) continue;
      ...
      await sleep(250); // public RPCs rate-limit bursts
      if (!tx) continue;
      const keys = tx.transaction.message.staticAccountKeys;
      for (const ix of tx.transaction.message.compiledInstructions) {
        if (!keys[ix.programIdIndex]?.equals(programId())) continue;
        const d = Buffer.from(ix.data);
        if (!d.subarray(0, 8).equals(tag)) continue;
        const [id, a] = readBorshString(d, 8);
        if (id !== drawId) continue;
        const [periodEndIso, b] = readBorshString(d, a);
        const [merkleRootHex, e] = readBorshString(d, b);
        return {
          ...
          periodEndIso,
          merkleRootHex,
          periodEndUnix: d.readBigInt64LE(e),
        };
      }
    }
