...
import {
  ...
  CLAIM_ROLL_PURCHASE_SLOT,
  rollKindEntropyBytes,
} from './gift-draw-registry-client.js';
import { rollFromPurchaseSig, purchasableKindFromRoll } from './kind-roll.js';
...
export function isNonRetriableKindRollError(message: string): boolean {
  ...
  return (
    ...
    m.includes('purchase_slot mismatch') ||
    m.includes('roll value does not match purchase tx slot')
  );
}
...
import { getPurchaseTxSlot, verifyPurchaseTxForRoll } from './verify-purchase-tx.js';
...
/** USD-denominated purchase currencies: purchase_amount is dollars, not GIFT. */
const USD_PURCHASE_CURRENCIES = new Set(['playdollar', 'usdc']);

/**
 * Refuse to attest a purchase whose GIFT value could not be resolved.
 *
 * For a USD-denominated buy the GIFT value lives ONLY in purchase_amount_gift_equiv (the pool quote).
 * If it is null, expectedGiftMicroFromRow silently falls back to purchase_amount — which is dollars —
 * and attests gift_amount_micro as if a $1 ticket were worth 1 GIFT. That number is emitted on-chain
 * and is canonical, so the mistake is permanent. Better to leave the ticket pending: the value is
 * backfilled from the pool and the next flush picks it up.
 *
 * Retriable by design — never mark it sync_error/non-retriable, or a transient RPC hiccup would
 * poison the ticket forever.
 */
export function missingGiftValueError(row: TicketRollRow): string | null {
  if (normalizeTicketOrigin(row.ticket_origin) !== TICKET_ORIGIN_PURCHASE) return null;
  const currency = String(row.purchase_currency || '').trim().toLowerCase();
  if (!USD_PURCHASE_CURRENCIES.has(currency)) return null;
  const gift = row.purchase_amount_gift_equiv;
  if (gift != null && Number.isFinite(Number(gift)) && Number(gift) > 0) return null;
  return `GIFT value unresolved for ${currency} purchase (pool quote unavailable) — not attesting, would emit a wrong gift_amount_micro`;
}
...
export function ticketMatchesKindRolledV2(row: TicketRollRow, parsed: ParsedKindRolledV2): string | null {
  ...
  // Rolls keep the registry version they were made under; v19–v21 share the roll formula (v21 only
  // changed draw seeds), so rolls from before and after each upgrade verify alike.
  const okRegistry =
    parsed.registryVersion <= SETTLEMENT_SPEC_VERSION_ON_CHAIN &&
    parsed.registryVersion >= SETTLEMENT_SPEC_VERSION_ON_CHAIN - 2;
  if (!okRegistry) {
...
/** Event slot must match purchase tx slot (or 0 for claim) — prevents execution-slot grinding. */
export async function verifyKindRollPurchaseSlot(
  row: TicketRollRow,
  parsed: ParsedKindRolledV2
): Promise<string | null> {
  const origin = normalizeTicketOrigin(row.ticket_origin);
  let expectedSlot: number;
  if (origin === TICKET_ORIGIN_CLAIM || parsed.origin === ROLL_ORIGIN_CLAIM) {
    expectedSlot = CLAIM_ROLL_PURCHASE_SLOT;
  } else {
    const pay = canonicalRollPurchaseTxSig(row.purchase_tx_sig, row.ticket_origin);
    if (!pay || pay.startsWith('claim:')) {
      return 'purchase_tx_sig missing for purchase-slot verification';
    }
    const slotRes = await getPurchaseTxSlot(pay);
    if (!slotRes.ok) return slotRes.error;
    expectedSlot = slotRes.slot;
  }

  // Use the authoritative on-chain entropy from the event itself, not a re-derivation of
  // row.purchase_tx_sig. For claim tickets the stored ref can drift from the bytes the program
  // actually hashed (reconciled prize ref), which made this slot check spuriously fail even though
  // the roll is valid — ticketMatches / claim-child already bind the entropy to the prize. This only
  // needs to bind the roll to the expected slot. For purchases parsed.purchaseTxSig equals the
  // 64-byte sig bytes, so behaviour is unchanged.
  const entropy = parsed.purchaseTxSig;

  const rollAtPurchaseSlot = rollFromPurchaseSig(
    entropy,
    parsed.ticketIndex,
    BigInt(expectedSlot)
  );
  if (parsed.slot === expectedSlot) {
    if (parsed.roll !== rollAtPurchaseSlot) {
      return 'roll value does not match purchase tx slot hash';
    }
    const expectedKind = purchasableKindFromRoll(rollAtPurchaseSlot);
    if (parsed.kind !== expectedKind) {
      return `kind does not match roll at purchase slot (expected ${expectedKind})`;
    }
    return null;
  }

  // Pre-upgrade rolls used execution slot in hash (legacy devnet tickets).
  const rollAtEventSlot = rollFromPurchaseSig(
    entropy,
    parsed.ticketIndex,
    BigInt(parsed.slot)
  );
  if (parsed.roll === rollAtEventSlot) {
    const expectedKind = purchasableKindFromRoll(rollAtEventSlot);
    if (parsed.kind === expectedKind) return null;
  }

  return `purchase_slot mismatch (expected ${expectedSlot}, event ${parsed.slot})`;
}
...
export async function verifyTicketKindRollOnChain(
  ...
  if (!err) {
    const slotErr = await verifyKindRollPurchaseSlot(row, parsed);
