...
/** `verifyTicketKindRollOnChain`'s error when the roll tx yields no event — also what an unreadable tx gives. */
const ROLL_TX_UNREADABLE = 'KindRolledV2 not found in roll transaction';
  ...
  for (let i = 0; i < rareForRpc.length; i += concurrency) {
    ...
    await Promise.all(
      slice.map(async (t) => {
        ...
        // Entry already verified this roll on-chain (kind_roll_verified_at). A roll transaction that
        // cannot be read back now is an RPC failure, not a forged roll — excluding would burn a paid
        // ticket (status 'used') for the node's sake. A roll that reads but does not match still
        // excludes.
        if (reason && reason.includes(ROLL_TX_UNREADABLE) && (t.kind_roll_verified_at || '').trim()) {
          console.warn('settlement: roll re-check could not read the roll tx; keeping the entry-verified ticket', t.id);
          verified.push(t);
          return;
        }
        if (reason) {
