...
/**
 * The first `n` of {@link weightedShuffle} — same rng calls in the same order, same keys, same
 * tie-break — keeping only `n` candidates instead of a keyed copy of every item. For draws whose
 * entrant list runs into the millions when only the top places are stored.
 */
export function weightedShuffleTop<T>(
  items: readonly T[],
  weightOf: (item: T) => number,
  rng: WeightedShuffleRng,
  shuffleSalt: string,
  ...
  const top: { item: T; key: number; idx: number }[] = [];
  const before = (a: { key: number; idx: number }, b: { key: number; idx: number }) =>
    a.key < b.key || (a.key === b.key && a.idx < b.idx);
  for (let idx = 0; idx < items.length; idx++) {
    const item = items[idx]!;
    const w = Math.max(MIN_WEIGHT, weightOf(item));
    const u = rng.nextOpenUnit(`${shuffleSalt}|${idx}`);
    const cand = { item, key: -Math.log(u) / w, idx };
    if (top.length === n && !before(cand, top[n - 1]!)) continue;
    let at = top.length;
    while (at > 0 && before(cand, top[at - 1]!)) at--;
    top.splice(at, 0, cand);
    if (top.length > n) top.pop();
  }
  return top.map((k) => k.item);
}
